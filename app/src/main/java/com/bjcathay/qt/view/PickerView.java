...
public class PickerView extends View {
    ...
    //设置团购日期
    public void setTuanPriceData(ProductModel datas) {
        String now = DateUtil.stringToDateToString(datas.getStart());
        mDataList.add(now);
        mCurrentSelected = 1;
        invalidate();
    }
    ...
    //设置日期
    public void setPriceData(List<PriceModel> datas) {
        for (PriceModel p : datas) {
            String now = DateUtil.stringToDateToString(p.getStartAt());
            mDataList.add(now);
        }
        mCurrentSelected = datas.size() / 2;
        invalidate();
    ...
    //设置时分
    public void setHourData(List<PriceModel> datas) {
        for (PriceModel p : datas) {
            String now = DateUtil.stringToDateToHourString(p.getStartAt());
            mDataList.add(p.getStartAt());
        }
        mCurrentSelected = datas.size() / 2;
        invalidate();
    }
    ...
    public interface onSelectListener {
        void onSelect(String text, int i);
    }
}
