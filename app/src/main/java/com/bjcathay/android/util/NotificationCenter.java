...
public class NotificationCenter {
    ...
    public synchronized void off(NotificationObserver observer) {
        for (Map.Entry<String, List<NotificationObserver>> entry : mapping.entrySet()) {
            List<NotificationObserver> value = entry.getValue();
            value.remove(observer);
        }
    }

    public synchronized void send(final String name, final Notification notification) {
        List<NotificationObserver> observers = mapping.get(name);
        ...
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            final List<NotificationObserver> clonedObservers = new ArrayList<NotificationObserver>(observers);
    ...
    private void fire(List<NotificationObserver> observers, String name, Notification notification) {
